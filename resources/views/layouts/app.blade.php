        ...
        <div class="flex min-h-screen flex-col md:flex-row">
            <aside class="bg-white shadow-md dark:bg-gray-950 md:w-64">
                <div class="flex h-full flex-col p-6">
                    <div class="mb-10">
                        <div class="text-2xl font-bold text-indigo-600">Gym Management</div>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Manage your gym</p>
                    </div>

                    <nav class="flex-1 space-y-2">
                        <a href="{{ route('members.index') }}"
                           class="flex items-center gap-3 rounded-lg px-4 py-2 text-sm font-medium transition hover:bg-indigo-50 hover:text-indigo-600
                           {{ request()->routeIs('members.*') ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-200' : 'text-gray-600 dark:text-gray-300' }}">
                            <span class="h-2 w-2 rounded-full bg-current"></span>
                            Gym Members
                        </a>
                        <a href="{{ route('plans.index') }}"
                           class="flex items-center gap-3 rounded-lg px-4 py-2 text-sm font-medium transition hover:bg-indigo-50 hover:text-indigo-600
                           {{ request()->routeIs('plans.*') ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-200' : 'text-gray-600 dark:text-gray-300' }}">
                            <span class="h-2 w-2 rounded-full bg-current"></span>
                            Gym Plans
                        </a>
                    </nav>
                    ...
                    <div class="mt-10 rounded-lg bg-gray-100 p-4 text-sm dark:bg-gray-800">
                        ...
                        <p class="text-gray-500 dark:text-gray-400">{{ auth()->user()?->email ?? 'admin@gym.com' }}</p>
            ...
            <div class="flex-1">
                <header class="flex items-center justify-between border-b border-gray-200 bg-white px-6 py-4 dark:border-gray-800 dark:bg-gray-950 md:hidden">
                    <div>
                        ...
                        <h1 class="text-xl font-semibold">{{ auth()->user()?->name ?? 'Admin' }}</h1>
                    </div>
